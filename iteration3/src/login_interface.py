...
import json
import sys
import session, colors, student_interface, advisor_interface, course, lecture, lab, course_session, advisor, student, lecturer
...
class LoginInterface:
    ...
    def login(self):
        while True:
            print(self.colors.get_bold() + self.colors.get_red() +
            ...
            if user_type == "student":
                if self.user_exists("students", ID):
                    if self.check_user_login_info("students", ID, password):
                        self.session = session.Session(self.create_student(ID, None))
                        print("\nWelcome {} {}!".format(
                            self.session.user.get_name(),
                            self.session.user.get_surname()))
                        ...
                        self.write_log("Student {} logged in.".format(
                            self.session.user.get_id()))
                        student_interface = student_interface(self.session, self)
                        student_interface.stu_menu()
                    else:
                        print("{}Invalid ID or password! Please try again.{}".format(
                            self.colors.get_yellow(),
                            self.colors.get_reset()))
                        continue
                else:
                    print("{}Invalid ID! Please try again.{}".format(
                        self.colors.get_yellow(),
                        self.colors.get_reset()))
                    continue
                ...
                if self.user_exists("lecturers", ID):
                    if self.check_user_login_info("advisors", ID, password):
                        self.session = session.Session(self.create_advisor(ID, "null"))
                        print("\nWelcome {} {}!".format(
                            self.session.user.get_name(),
                            self.session.user.get_surname()))
                        print("You have {} successfully {} logged in.".format(
                        ...
                        self.write_log("Advisor {} logged in.".format(
                            self.session.user.get_id()))
                        advisor_interface = advisor_interface(self.session, self)
                        advisor_interface.adv_menu()
                    elif self.check_user_login_info("lecturers", ID, password):
                        self.session = session.Session(self.create_lecturer(ID))
                        print("\nWelcome {} {}!".format(
                            self.session.user.get_name(),
                            self.session.user.get_surname()))
                        print("You have {} successfully {} logged in.".format(
                        ...
                        self.write_log("Lecturer {} logged in.".format(
                            self.session.user.get_id()))
                        advisor_interface = advisor_interface(self.session, self)
                        advisor_interface.adv_menu()
                    else:
    ...
    def logout(self):
        while True:
            print("Do you want to logout and exit? ({}/{})".format(
                self.colors.get_yellow() + "y" + self.colors.get_reset(),
                self.colors.get_yellow() + "n" + self.colors.get_reset()))
            answer = input()
            if answer == "y":
                print("You have {} successfully {} logged out and exited.".format(
                    self.colors.get_green(),
                    self.colors.get_reset()))
                print("{}{}Thank you for using Marmara Course Registration System{}".format(
                    self.colors.get_red(),
                    self.colors.get_bold(),
                    self.colors.get_reset()))
                self.write_log("User {} logged out.".format(
                    self.session.user.get_id()))
                self.session.user = None
                self.exit()
            elif answer == "n":
                print("{}You will be redirected to the login menu.{}".format(
                    self.colors.get_yellow(),
                    self.colors.get_reset()))
                self.session.user = None
                self.login()
            else:
                print("{}Invalid input! Please try again.{}".format(
                    self.colors.get_yellow(),
                    self.colors.get_reset()))
                continue
    ...
    def create_student(self, ID, advisor):
        try:
            with open("jsons/student/{}.json".format(ID), 'r') as reader:
                ...
                stu = student.Student(name, surname, email, phone_number, student_ID,
                              password, faculty, department, semester, advisor)
            ...
            with open("jsons/advisors.json", 'r') as reader:
                ...
                for advisor in advisor_list:
                    if advisor["ID"] == advisor_ID:
                        ...
                        adv = advisor.Advisor(name, surname, email, phone_number, advisor_ID,
                                      password, faculty, department, academic_title)
    ...
    def create_lecturer(self, lecturer_ID):
        try:
            with open("jsons/lecturers.json", 'r') as reader:
                lecturer_list = json.load(reader)
                ...
                for lecturer in lecturer_list:
                    if lecturer["ID"] == lecturer_ID:
                        ...
                        lec = lecturer.Lecturer(name, surname, email, phone_number, lecturer_ID,
                                       password, faculty, department, academic_title)
            ...
            with open("jsons/CoursesOffered.json", 'r') as reader:
                ...
                for course_obj in courses_list:
                    if course_obj["CourseID"] == course_code:
                        ...
                        course_session = self.create_course_session(course_day_time_location)

                        dot_count = course_code.count('.')
                        if dot_count == 1:
                            return lecture.Lecture(course_name, course_code, course_session)
                        elif dot_count == 2:
                            return lab.Lab(course_name, course_code, course_session)
                        elif dot_count == 0:
                            return course.Course(course_name, course_code, course_session)
                        else:
                            print("{}Invalid course code!{}".format(
                                self.colors.get_cyan(),
                                self.colors.get_reset()))
                            continue
        except (IOError, json.JSONDecodeError):
            pass
    ...
    def create_course_session(self, course_day_time_location):
        ...
        return course_session.CourseSession(course_day, course_start_time, course_end_time, course_place)

    def id_format_checker(self, ID):
